CREATE OR REPLACE VIEW public.view_on_treatment
AS
SELECT c.clinic_id,
       ...
       ( SELECT string_agg(pti.has_concurrent_chemotherapy::text, ','::text) AS has_concurrent_chemo
         FROM patient_treatment_info pti
         WHERE pti.patient_tracker_id = pt.patient_tracker_id) AS has_concurrent_chemo,
       wf.name AS workflow
...
WHERE ws.step::text = 'ON_TREATMENT'::text;
